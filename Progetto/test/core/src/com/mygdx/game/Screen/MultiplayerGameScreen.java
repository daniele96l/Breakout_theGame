...
//disegnoimattoncini
        ...
        game.getBatch().draw(paddles.get(1), paddles.get(1).getPosition().x, paddles.get(1).getPosition().y, paddles.get(1).getWidth() * Info.paddleresize, paddles.get(1).getHeight() * Info.paddleresize);
        commandPlayers.get(1).move();
    ...
    public void gestisciCollisioni() {
        ...
        if(collision.checkSide(paddles.get(1))) {
            gameHolder = players.get(1);
            brickCounter = 0;
            }
    ...
    public static void setPlayerName(String playerName) {
        MultiplayerGameScreen.playerName = playerName;
    }

    private void lostLife(float positionX) {
        int range=Info.larghezza/2;
    ...
    private void updateScene() {
        ...
        for(Paddle paddle:paddles) {
            paddle.setDefaultState(2);
        }
        commandPlayers=new ArrayList<CommandPlayer>();
        for(int i=0; i<players.size(); i++) {
            commandPlayers.add(new CommandPlayer(paddles.get(i), players.get(i), 2, i+1));
        }
    }
