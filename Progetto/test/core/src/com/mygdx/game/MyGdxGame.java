...
public void render(){
       ...
       //
        //dato=new Dato(palla,paddle1,mattoncini);
        // paddle1.setPositionM(chatClient.start_main(dato));

        if(nextLevel){//devestaredentrorenderperchèdeveesserecontrollatosempre
        mattoncini=livello.selectLv();//ritornol'arrayadattoalnuovolivello
        bg=livello.getBg();//reimpostoilbg
...
public void dispose(){
        ...
        mattoncini=livello.selectLv();//laclasselivellosioccuperàdiritornarel'arraylistdeimattonciniadattiaquestolivello
        bg=livello.getBg();
        }
        ...
        for(AbstractBrick Abstractbrick:mattoncini){
//datochehounarraylistdevoaggiornarelecondizionideimattoncinidentrounciclofor
        ...
        for(int i:indici){
        if(mattoncini.get(indici.get(0)).getDurezza()==0){//imattoncinivengonoeliminatisolosesonoquelliMORBIDI
        ...
        if(mattoncini.get(indici.get(0)).getDurezza()==0){//seimattoncinisonomattoncini"morbidi"lipossoeliminare
